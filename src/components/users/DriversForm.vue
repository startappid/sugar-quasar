<template>
  <div class="q-pa-md shadow-1">
    ...
    <div class="row q-gutter-md q-my-md">
      <div class="col-3"  v-if="stateForm=='show'">
        <q-select
          clearable
          :readonly="readonly"
          filled
          ...
          emit-value
          map-options
          @filter="filterUsersOpt"
          hint="Mininum 1 characters to trigger filtering"
        >
...
export default {
  ...
  mounted () {
    if (this.stateForm === 'show' || this.stateForm === 'update') {
      ...
      this.detail(data).then((response) => {
        const { data } = response
        this.form = {
          ...
          role: 'driver' // FIXME: hardcoded
        }
        this.loading = false
        this.detailUser({ id: data.updated_by }).then((response) => {
          const { data } = response
  ...
  methods: {
    ...
    filterUsersOpt (val, update, abort) {
      if (val.length < 1) {
        abort()
        return
      }
      ...
      this.fetchUsers(params).then((response) => {
        update(() => {
          const { data } = response
          this.userOptions = data
        })
