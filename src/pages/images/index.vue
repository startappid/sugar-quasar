  ...
  <q-page class="q-pa-md">
    ...
    >
      <template v-slot:body-cell-image="props">
        <q-td :props="props" class="cursor-pointer" @click="popupImage(props.row)">
          <q-avatar rounded size="100px">
            <img :src="props.row.fullpath" />
          </q-avatar>
        </q-td>
      </template>
      ...
      <template v-slot:body-cell-information="props">
        <q-td :props="props">
          ...
          Dimensions: {{props.row.dimension_width}}x{{props.row.dimension_height}} <br>
        </q-td>
      </template>
    </DataTable>

    <q-dialog v-model="openPopup" full-width>
      <q-card v-close-popup>
        <q-img :src="imgUrl" />
      </q-card>
    </q-dialog>
